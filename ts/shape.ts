...
const isVec3Array = (value: TVec3Like): value is readonly [number, number, number] =>
	Array.isArray(value);

const isQuatArray = (value: TQuatLike): value is readonly [number, number, number, number] =>
	Array.isArray(value);
...
const initShape = ({ scene, three }: TInitShapeOpts): TNewableShape => {
	class Shape extends Body {
		...
		public get debug(): TDebugMode | null {
			return this._debug;
		}
